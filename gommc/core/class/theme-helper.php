    ...
    class GoMMC_Theme_Helper
    {
        ...
        public static function get_option(String $name)
        {
            if (class_exists('Redux')) {
                // Customizer
                if (!empty($GLOBALS['gommc_set'])) {
                    $theme_options = $GLOBALS['gommc_set'];
                } else {
                    $theme_options = get_option('gommc_set');
                }
            }

            if (empty($theme_options)) {
                $theme_options = get_option('gommc_default_options');
            }
        ...
        {
            return [
               'before' => '<div class="tpc-pagination"><span class="tpc-pagination-text">' . __( 'Pages:', 'gommc' ) . '&nbsp;</span>',
                'after' => '</div>',
        ...
        public static function load_more(
            ...
            $load_more_text = !empty($load_more_text) ? $load_more_text : esc_html__('Load More', 'gommc');
        ...
        public static function get_sidebar_data($name = 'page')
        {
	        ...
	        $name = apply_filters('gommc/theme-helper/get-sidebar-params/cpt', $name);
        ...
        public static function render_sidebar(Array $sb_data)
        {
            ...
            if (is_active_sidebar($sb_data['id'])) {
                echo '<aside class="sidebar">';
                    do_action('gommc/theme-helper/render-sidebar/before-widgets');
        ...
        public static function posted_meta_on()
        {
            ...
            printf(
                '<span><time class="entry-date published" datetime="%1$s">%2$s</time></span><span>' . esc_html__('Published in', 'gommc') . ' <a href="%3$s" rel="gallery">%4$s</a></span>',
            ...
            if ($metadata) {
                printf(
                    ...
                    esc_html_x('Full size', 'Used before full size attachment link.', 'gommc'),
            ...
            edit_post_link(
                ...
                sprintf(
                    wp_kses(__('Edit<span class="screen-reader-text"> "%s"</span>', 'gommc'), $kses_allowed_html),
                    get_the_title()
                ),
        ...
        public static function enqueue_css($style)
        {
            ...
            $css = ob_get_clean();
            $css = apply_filters('gommc/enqueue_shortcode_css', $css, $style);
        }
