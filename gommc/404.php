 ...
 * @since 1.0.0
 */
$layout_template = GoMMC_Theme_Helper::get_option('404_page_type');
if ($layout_template == 'custom') {
    $page_select = GoMMC_Theme_Helper::get_option('404_template_select');
  ...
  $primary_color = esc_attr(GoMMC_Theme_Helper::get_option('theme-primary-color'));
  $h_font_color = esc_attr(GoMMC_Theme_Helper::get_option('header-font')['color']);

  $bg_color = GoMMC_Theme_Helper::get_option('404_page_main_bg_image')['background-color'];
  $bg_image = GoMMC_Theme_Helper::bg_render('404_page_main');
  ...
  // Particles
  $particles = GoMMC_Theme_Helper::get_option('404_particles');
  if ($particles) {
...
get_header();
if ($layout_template == 'custom') {
  echo GoMMC_Theme_Helper::render_html($render_template);
} else { ?>

<div class="tpc-container full-width">
  <div class="row">
    <div class="tpc_col-12">
      <section class="page_404_wrapper" <?php echo GoMMC_Theme_Helper::render_html($styles); ?>>
        <div class="page_404_wrapper-container">
          <div class="row">
            <div class="tpc_col-12 tpc_col-md-12">
              <div class="main_404-wrapper">
                <div class="banner_404">
                  <img src="<?php echo esc_url(get_template_directory_uri() . "/img/404.png"); ?>" alt="<?php echo esc_attr__('404', 'gommc'); ?>">
                </div>
                <h2 class="banner_404_title"><span><?php esc_html_e('Sorry We Can`t Find That Page!', 'gommc'); ?></span></h2>
                <p class="banner_404_text">
                  <?php esc_html_e('The page you are looking for was moved, removed, renamed or never existed.', 'gommc'); ?>
                </p>
                <div class="gommc_404_search">
                  <?php get_search_form(); ?>
                </div>
        ...
        <?php if ($particles) : ?>
          <div id="<?php echo uniqid('page_404'); ?>"
            ...
            data-particles-color="<?php echo GoMMC_Globals::get_primary_color(), ',', GoMMC_Globals::get_h_font_color();?>"
