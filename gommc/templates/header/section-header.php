...
if (!class_exists('GoMMC_Get_Header')) {
    class GoMMC_Get_Header
    {
        ...
        public function header_vars()
        {
            ...
            * @since 1.0.0
            */
            $this->header_type = GoMMC_Theme_Helper::get_option('header_type');

            if ($this->header_type === 'custom') {

                $header_page_select = GoMMC_Theme_Helper::get_option('header_page_select');
            ...
            */
            ...
            if (GoMMC_Theme_Helper::get_mb_option('header_sticky', 'mb_customize_header_layout', 'custom') == '1') {
                $header_sticky_page_select = GoMMC_Theme_Helper::get_option('header_sticky_page_select');
        ...
        {
            $header_shadow = GoMMC_Theme_Helper::get_option('header_shadow');
            $header_on_bg = GoMMC_Theme_Helper::get_option('header_on_bg');
            $header_class = '';
        ...
        public function header_bar_editor($location = null, $position = null)
        {
            ...
            */
            ...
            ${'header_'.$position.'_editor'} = GoMMC_Theme_Helper::get_option($location.'_header_bar_'.$position.'_editor');
        ...
        public function header_bar_delimiter($k = null)
        {
            ...
            */
            ...
            $height = GoMMC_Theme_Helper::get_option('bottom_header_delimiter'.$get_number.'_height');
            $width = GoMMC_Theme_Helper::get_option('bottom_header_delimiter'.$get_number.'_width');

            $bg_color = GoMMC_Theme_Helper::get_option('bottom_header_delimiter'.$get_number.'_bg');

            $margin = GoMMC_Theme_Helper::get_option('bottom_header_delimiter'.$get_number.'_margin');
            ...
            if ($this->html_render === 'sticky') {
                $custom_sticky = GoMMC_Theme_Helper::get_option('bottom_header_delimiter'.$get_number.'_sticky_custom');
                if (!empty($custom_sticky)) {
                    $bg_color = GoMMC_Theme_Helper::get_option('bottom_header_delimiter'.$get_number.'_sticky_color');
                    $height  = GoMMC_Theme_Helper::get_option('bottom_header_delimiter'.$get_number.'_sticky_height');
                }
            }
        ...
        public function header_bar_button($k = null)
        {
            ...
            $get_number = (int) filter_var($k, FILTER_SANITIZE_NUMBER_INT);
            $button_text = GoMMC_Theme_Helper::get_option('bottom_header_button'.$get_number.'_title');

            $link = GoMMC_Theme_Helper::get_option('bottom_header_button'.$get_number.'_link');
            $target = GoMMC_Theme_Helper::get_option('bottom_header_button'.$get_number.'_target');

            $size = GoMMC_Theme_Helper::get_option('bottom_header_button'.$get_number.'_size') ?? 'md';

            $customize = GoMMC_Theme_Helper::get_option('bottom_header_button'.$get_number.'_custom');
            $customize = empty($customize) ? 'def' : 'color';

            $text_color_idle = GoMMC_Theme_Helper::get_option('bottom_header_button'.$get_number.'_color_txt')['rgba'] ?? '';
            $text_color_hover = GoMMC_Theme_Helper::get_option('bottom_header_button'.$get_number.'_hover_color_txt')['rgba'] ?? '';

            $border_color = GoMMC_Theme_Helper::get_option('bottom_header_button'.$get_number.'_border')['rgba'] ?? '';
            $border_color_hover = GoMMC_Theme_Helper::get_option('bottom_header_button'.$get_number.'_hover_border')['rgba'] ?? '';
            $border_radius = GoMMC_Theme_Helper::get_option('bottom_header_button'.$get_number.'_radius') ?? false;

            $bg_color = GoMMC_Theme_Helper::get_option('bottom_header_button'.$get_number.'_bg')['rgba'] ?? '';
            $bg_color_hover = GoMMC_Theme_Helper::get_option('bottom_header_button'.$get_number.'_hover_bg')['rgba'] ?? '';

            $button_css_id = uniqid('gommc_button_');
            ...
            // Register css
            if ($styles) GoMMC_Theme_Helper::enqueue_css($styles);
        ...
        public function get_render_attribute_string($element)
        {
            ...
            return ' ' . GoMMC_Theme_Helper::render_html_attributes( $this->render_attributes[ $element ] );
        }
        ...
        */
        public function header_bar_spacer($location = null, $key = null)
        {
            ...
            $spacer = GoMMC_Theme_Helper::get_option($location.'_header_spacer'.$get_number);
        ...
        {
            $header_layout = GoMMC_Theme_Helper::get_option($section . '_header_layout');
            $lavalamp_active = GoMMC_Theme_Helper::get_option('lavalamp_active');
                                ...
                                if (
                                    ...
                                    foreach ($header_layout[$part] as $key => $value) if ($key != 'placebo') {
                                        switch ($key) {
                                            ...
                                            case 'menu':
                                                $menu = 'main_menu';
                                                if (GoMMC_Theme_Helper::get_option('custom_mobile_menu')) {
                                                    $custom_menu = true;
                                                    $menu = GoMMC_Theme_Helper::get_option('mobile_menu');
                                                }
                                                if (
                                                    class_exists('RWMB_Loader')
                                                    && $this->id !== 0
        ...
        */
        private function row_width_class($s = '_middle', $section)
        {
            ...
            switch ($section) {
                case 'bottom':
                    $width_container = GoMMC_Theme_Helper::get_option('header'.$s.'_full_width');
                    break;
                case 'sticky':
                    $width_container = GoMMC_Theme_Helper::get_option('header_custom_sticky_full_width');
                    break;
                case 'mobile':
                    $width_container = GoMMC_Theme_Helper::get_option('header_mobile_full_width');
                    break;
            }
        ...
        private function row_style_color($s = '_middle', $section)
        {
            ...
            switch ($this->html_render) {
                case 'bottom':
                case 'sticky':
                    $header_background = GoMMC_Theme_Helper::get_option('header'.$s.'_background');
                    $header_bg_image = GoMMC_Theme_Helper::get_option('header'.$s.'_background_image')['url'] ?? '';

                    $header_color = GoMMC_Theme_Helper::get_option('header'.$s.'_color');
                    $header_bottom_border = GoMMC_Theme_Helper::get_option('header'.$s.'_bottom_border');
                    $header_border_height = GoMMC_Theme_Helper::get_option('header'.$s.'_border_height')['height'] ?? '';
                    $header_bottom_border_color = GoMMC_Theme_Helper::get_option('header'.$s.'_bottom_border_color');
                    ...
                    $customize_width = GoMMC_Theme_Helper::get_option('header'.$s.'_max_width_custom');

                    if ($customize_width == '1') {
                        $max_width = GoMMC_Theme_Helper::get_option('header'.$s.'_max_width');
        ...
        */
        ...
        {
            $header_mobile_height = GoMMC_Theme_Helper::get_option('header_mobile_height')['height'] ?? false;
            $header_height = GoMMC_Theme_Helper::get_option('header'.$s.'_height')['height'] ?? false;
        ...
        {
            $icon_bg = GoMMC_Theme_Helper::get_option('bottom_header_side_panel_background')['rgba'] ?? '';
            $icon_color = GoMMC_Theme_Helper::get_option('bottom_header_side_panel_color')['rgba'] ?? '';
        ...
        {
            $dispay = GoMMC_Theme_Helper::get_option('header_column' . $s . '_' . $area . '_display');
            $v_align = GoMMC_Theme_Helper::get_option('header_column' . $s . '_' . $area . '_vert');
            $h_align = GoMMC_Theme_Helper::get_option('header_column' . $s . '_' . $area . '_horz');
        ...
        {
            $header_queries = GoMMC_Theme_Helper::get_option('header_mobile_queris');
            ...
            $menu_occurrence = $mobile_header_custom ? GoMMC_Theme_Helper::get_option('mobile_position') : '';
        ...
        public function get_logo($menu_condition = false, $_prefix = '')
        {
            ...
            new GoMMC_Get_Logo($location, $menu_condition, $logo ?? '', $custom_size ?? '');
        }
        ...
        {
            $description = esc_html__('Type To Search', 'gommc');
            $search_style = GoMMC_Theme_Helper::get_option('search_style') ?: 'standard';
            ...
            $customize = GoMMC_Theme_Helper::get_option('bottom_header_item_search_custom');
            $customize = empty($customize) ? 'def' : 'color';

            $text_color = GoMMC_Theme_Helper::get_option('bottom_header_item_search_color_txt')['rgba'] ?? '';
            $text_color_hover = GoMMC_Theme_Helper::get_option('bottom_header_item_search_hover_color_txt')['rgba'] ?? '';

            $search_css_id = uniqid('gommc_search_');
            ...
            // Register css
            if (!empty($styles)) {
                GoMMC_Theme_Helper::enqueue_css($styles);
            }
                    ...
                    echo '<div class="header_search-field">';
                        if ($search_style === 'alt') {
                            echo '<div class="header_search-wrap">',
                                '<div class="gommc_module_double_headings">',
                                    '<h3 class="header_search-heading_description heading_title">',
                                        apply_filters('gommc/search/description', $description),
                                    '</h3>',
                                '</div>',
                                '<div class="header_search-close flaticon-close"></div>',
                            '</div>';
        ...
        */
        ...
        public function login_in($location, $section)
        {
            ...
            echo "<div class='login-in woocommerce'", $this->row_style_height($location, $section), '>';
                ...
                if (is_user_logged_in()) {
                    echo "<a class='login-in_link-logout' href='", esc_url($link_logout), "'>", esc_html__('WC LOGOUT', 'gommc'), "</a>";
                } else {
                    echo "<a class='login-in_link' href='", esc_url_raw($url), "'>", esc_html__('WC LOGIN', 'gommc'), '</a>';
                }
        ...
        $login_page_url =  GoMMC_Theme_Helper::get_option('login_page_url_mobile');
        $join_page_url = GoMMC_Theme_Helper::get_option('join_page_url_mobile');
        $login_text = GoMMC_Theme_Helper::get_option('login_text_mobile');
        $join_text = GoMMC_Theme_Helper::get_option('join_text_mobile');
        $logout_text = GoMMC_Theme_Helper::get_option('logout_text_mobile');
       ...
       if (!is_user_logged_in()) :
        ...
        {
            $customize = GoMMC_Theme_Helper::get_option('bottom_header_cart_custom');
            $customize = empty($customize) ? 'def' : 'color';

            $text_color_idle = GoMMC_Theme_Helper::get_option('bottom_header_catpc_color_txt')['rgba'] ?? '';
            $text_color_hover = GoMMC_Theme_Helper::get_option('bottom_header_cart_hover_color_txt')['rgba'] ?? '';

            $cart_css_id = uniqid('gommc_woo_');
            ...
            $this->add_render_attribute('cart', 'title', esc_attr__('Click to open Shopping Cart', 'gommc'));
