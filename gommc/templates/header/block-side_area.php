...
if (! class_exists('GoMMC_Header_Side_Area')) {
    class GoMMC_Header_Side_Area extends GoMMC_Get_Header
    {
        public function __construct()
        {
            $this->header_vars();

            $side_panel_enable = GoMMC_Theme_Helper::get_option('side_panel_enable');

            if (empty($side_panel_enable)) {
                return;
            }

            $pos = GoMMC_Theme_Helper::get_mb_option('side_panel_position', 'mb_customize_side_panel', 'custom');

            $content_type = GoMMC_Theme_Helper::get_mb_option('side_panel_content_type','mb_customize_side_panel','custom');
            $class = !empty($pos) ? ' side-panel_position_'.$pos : ' side-panel_position_right';

            // Get options
            $side_panel_spacing = GoMMC_Theme_Helper::get_mb_option('side_panel_spacing','mb_customize_side_panel','custom');
        ...
        {
            $bg = GoMMC_Theme_Helper::get_option('side_panel_bg')['rgba'] ?? '';
            $color = GoMMC_Theme_Helper::get_option('side_panel_text_color')['rgba'] ?? '';
            $width = GoMMC_Theme_Helper::get_option('side_panel_width')['width'] ?? '';

            $align = GoMMC_Theme_Helper::get_mb_option('side_panel_text_alignment', 'mb_customize_side_panel', 'custom');
            $style = '';
        ...
        {
            $page_select = GoMMC_Theme_Helper::get_mb_option('side_panel_page_select', 'mb_customize_side_panel', 'custom');
