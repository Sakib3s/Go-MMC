...
if (!class_exists('GoMMC_Get_Page_Title')) {
    /**
     *  Page Title Area
     *
     *  @package gommc\templates
     *  @author MMC 
     *  @since 1.0.0
     */
    class GoMMC_Get_Page_Title
    {
        ...
        public function get_page_header_attributes()
        {
            ...
            // Parallax
            $parallax_enabled = GoMMC::get_mb_option('page_title_parallax', 'mb_page_title_switch', 'on');
            if ($parallax_enabled) {
                wp_enqueue_script('jquery-paroller', get_template_directory_uri() . '/js/jquery.paroller.min.js');
                $parallax_class = ' page_title_parallax';
                $parallax_speed = apply_filters('gommc/page_title/parallax_speed', GoMMC::get_mb_option('page_title_parallax_speed', 'mb_page_title_switch', 'on'));
        ...
        public function get_page_header_classes()
        {
            if (
                ...
                if ($this->RWMB_is_active()) {
                    ...
                    $page_title_align = GoMMC::get_option($post_type . '_single_title_align');
                    $breadcrumbs_align = GoMMC::get_option($post_type . '_single_breadcrumbs_align');
                    $breadcrumbs_block = GoMMC::get_option($post_type . '_single_breadcrumbs_block_switch');
                }
            } else {
                $page_title_align = GoMMC::get_mb_option('page_title_align', 'mb_page_title_switch', 'on');
                $breadcrumbs_align = GoMMC::get_mb_option('page_title_breadcrumbs_align', 'mb_page_title_switch', 'on');
                $breadcrumbs_block = GoMMC::get_mb_option('page_title_breadcrumbs_block_switch', 'mb_page_title_switch', 'on');
            }
        ...
        {
            $bg_enabled = GoMMC::get_option($this->post_type . '_' . $this->post_query . '__page_title_bg_switch')
                ?? GoMMC::get_option('page_title_bg_switch');

            $min_height = GoMMC::get_option($this->post_type . '_' . $this->post_query . '__page_title_height')['height'] ?? 'px';
            $min_height = 'px' !== $min_height ? $min_height : GoMMC::get_option('page_title_height')['height'];

            $bg_color = GoMMC::get_option($this->post_type . '_' . $this->post_query . '__page_title_bg_image')['background-color'] ?? '';
            $bg_color = $bg_color ?: GoMMC::get_option('page_title_bg_image')['background-color'];

            $paddings = GoMMC::get_option($this->post_type . '_' . $this->post_query . '__page_title_padding') ?: [];
            if (!isset($paddings['padding-top']) || '' === $paddings['padding-top']) {
                $paddings['padding-top'] = GoMMC::get_option('page_title_padding')['padding-top'] ?? '';
            }
            if (!isset($paddings['padding-bottom']) || '' === $paddings['padding-bottom']) {
                $paddings['padding-bottom'] = GoMMC::get_option('page_title_padding')['padding-bottom'] ?? '';
            }

            $margin_bottom = GoMMC::get_option($this->post_type . '_' . $this->post_query . '__page_title_margin')['margin-bottom'] ?? '';
            $margin_bottom = '' !== $margin_bottom ? $margin_bottom : GoMMC::get_option('page_title_margin')['margin-bottom'];
        ...
        {
            $bg_enabled = GoMMC::get_option('page_title_bg_switch');
            $bg_color = GoMMC::get_option('page_title_bg_image')['background-color'];
            $min_height = GoMMC::get_option('page_title_height')['height'];
            $paddings = GoMMC::get_option('page_title_padding');
            $margin_bottom = GoMMC::get_option('page_title_margin')['margin-bottom'] ?? '';
        ...
        {
            if (is_404()) {
                return GoMMC::bg_render('404_page__page_title') ?: GoMMC::bg_render('page_title');
            }

            if (
                function_exists('is_woocommerce') && is_woocommerce()
                && !empty($bg_shop = GoMMC::bg_render($this->post_type . '_' . $this->post_query . '_page_title'))
            ) {
                return !is_product()
                    ? $bg_shop
                    : GoMMC::bg_render('shop_single_page_title');
            }

            if (
                $this->post_type
                && !empty($bg_cpt = GoMMC::bg_render($this->post_type . '_' . $this->post_query . '__page_title'))
            ) {
                return $bg_cpt;
            }

            return GoMMC::bg_render('page_title', 'mb_page_title_switch', 'on');
        }
        ...
        public function get_title_text()
        {
            if (is_home() || is_front_page()) {
                ...
                $title = single_term_title('', false) . esc_html__(' Tag', 'gommc');
            } elseif (is_date()) {
                $title = get_the_time('F Y');
            } elseif (is_author()) {
                $title = esc_html__('Author:', 'gommc') . ' ' . get_the_author();
            } elseif (is_search()) {
                $title = esc_html__('Search', 'gommc');
            } elseif (is_404()) {
                $this->page_title_tag = 'h1';
                $title = GoMMC::get_option('404_custom_title_switch')
                    ? esc_html(GoMMC::get_option('404_page_title_text'))
                    : esc_html__('Error Page', 'gommc');
            } elseif (is_singular('portfolio')) {
                $title = GoMMC::get_option('portfolio_title_conditional')
                    ? esc_html__('Portfolio', 'gommc')
                    : esc_html(get_the_title());
                $title = apply_filters('gommc/page_title/portfolio_single', $title);
            } elseif (is_singular('team')) {
                $title = GoMMC::get_option('team_title_conditional')
                    ? esc_html__('Team', 'gommc')
                    : esc_html(get_the_title());
                $title = apply_filters('gommc/page_title/team_single', $title);
            } elseif (function_exists('is_product') && is_product()) {
                $title = GoMMC::get_option('shop_title_conditional')
                    ? esc_html__('Shop', 'gommc')
                    : esc_html(get_the_title());
                $title = apply_filters('gommc/page_title/shop_single', $title);
            } elseif (is_archive()) {
                if (function_exists('is_shop')
                    ...
                    $title = esc_html__('Portfolio', 'gommc');
                    $title = apply_filters('gommc/page_title/portfolio_archive', $title);
                } else {
                ...
                if (!empty($post)) {
                    ...
                    if ('post' == get_post_type($post)) {
                        ...
                        $title = GoMMC::get_option('blog_title_conditional')
                            ? esc_html__('Blog Post', 'gommc')
                            : esc_html(get_the_title($post->ID));
                        $title = apply_filters('gommc/page_title/blog_single', $title);
                    } else {
                        $title = esc_html(get_the_title($post->ID));
                    }
                } else {
                    $title = esc_html__('No Posts', 'gommc');
                }
            }
            ...
            if (
                ...
                $title = apply_filters('gommc/page_title/custom_text', $title);
            }
        ...
        {
            $user_tag = GoMMC::get_mb_option('page_title_tag', 'mb_page_title_switch', 'on');
        ...
        {
            $pt_font = GoMMC::get_mb_option('page_title_font', 'mb_page_title_switch', 'on');
        ...
        public function is_breadcrumbs_enabled()
        {
            if (
                ...
                return 'on' == $this->mb_page_title_switch
                    ...
                    : GoMMC::get_option('blog_single_page_title_breadcrumbs_switch');
            }

            return GoMMC::get_mb_option('page_title_breadcrumbs_switch', 'mb_page_title_switch', 'on');
        }

        public function get_breadcrumbs()
        {
            $breadcrumbs_font = GoMMC::get_mb_option('page_title_breadcrumbs_font', 'mb_page_title_switch', 'on');
