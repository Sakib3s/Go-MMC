...
if (!class_exists('GoMMC_Breadcrumbs')) {
    /**
     * Breadcrumbs area
     *
     *
    ...
    class GoMMC_Breadcrumbs
    {
        ...
        {
            $this->text_home     = esc_html__('Home', 'gommc'); // text for the `Home` link
            $this->text_category = esc_html__('Archive by Category "%s"', 'gommc'); // text for a category page
            $this->text_search   = esc_html__('Search Results for "%s"', 'gommc'); // text for a search results page
            $this->text_tag      = esc_html__('Posts Tagged "%s"', 'gommc'); // text for a tag page
            $this->text_author   = esc_html__('Articles Posted by %s', 'gommc'); // text for an author page
            $this->text_404      = esc_html__('Error 404', 'gommc'); // text for the 404 page
            $this->text_page     = esc_html__('Page %s', 'gommc'); // text 'Page N'
            $this->text_cpage    = esc_html__('Comment Page %s', 'gommc'); // text `Comment Page N`
        }
        ...
        public function __construct ()
        {
            ...
            if ( is_home() || is_front_page() ) {
	            ...
	            if( class_exists( 'WooCommerce' ) && ( is_product() || is_product_category() || is_product_tag() || is_cart() || is_checkout())){
		            ...
		            $out .= gommc_woocommerce_breadcrumb($args);
                ...
                // Portfolio Taxonomies Archive
                    ...
                    $archive_link = get_post_type_archive_link(GoMMC_Theme_Helper::get_option('portfolio_slug') ?: 'portfolio');
                    $is_category = is_tax('portfolio-category') ? true : false;
                    $tax_name = get_queried_object()->name;

                    if (get_query_var('paged')) {
                        $out .= $sep . sprintf($link, $archive_link, esc_html__('Portfolio', 'gommc')) . $sep
                            . $before
                            . ($is_category ? sprintf($this->text_category, $tax_name) : sprintf($this->text_tag, $tax_name))
                            . ', ' . sprintf($this->text_page, get_query_var('paged'))
                            . $after;
                    } else {
                        if ($show_current) $out .= $sep . sprintf($link, $archive_link, esc_html__('Portfolio', 'gommc')) . $sep
                            . $before
                            . ($is_category ? sprintf($this->text_category, $tax_name) : sprintf($this->text_tag, $tax_name))
                            . $after;
                    }
                ...
                // Custom Post Type
                    ...
                    function_exists('is_product') && is_product()
                    && function_exists('gommc_woocommerce_breadcrumb')
                ) {
                    $args = [];
                    $args['delimiter'] = $sep;
                    if ($show_home_link) $out .= $sep;
            ...
            echo GoMMC_Theme_Helper::render_html($out);
        }
    }

    new GoMMC_Breadcrumbs();
}
