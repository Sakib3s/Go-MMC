...
if (!class_exists('GoMMC_Footer_Area')) {
    /**
     * Footer Area
     *
     *
    ...
    class GoMMC_Footer_Area
    {
        ...
        {
            $footer_options = apply_filters('gommc/footer/enable', true);
            extract($footer_options);

            $footer_bg_color = GoMMC_Theme_Helper::get_option('footer_bg_color');
            $style = '';

            // Upper-level variables
            $this->id = get_queried_object_id();
            $this->content_type = GoMMC_Theme_Helper::get_mb_option('footer_content_type', 'mb_footer_switch', 'on');
            $this->footer_full_width = GoMMC_Theme_Helper::get_option('footer_full_width');
            ...
            if (
                ...
                $style .= GoMMC_Theme_Helper::bg_render('footer', 'mb_footer_switch');
        ...
        private function get_footer_vars($optn_1 = null)
        {
            ...
            // Get options
            $footer_spacing = GoMMC_Theme_Helper::get_mb_option('footer_spacing', 'mb_footer_switch', 'on');
            $footer_border = GoMMC_Theme_Helper::get_mb_option('footer_add_border', 'mb_footer_switch', 'on');
            $footer_border_color = GoMMC_Theme_Helper::get_mb_option('footer_border_color', 'mb_footer_switch', 'on');

            if ($optn_1 == 'widgets') {
                $footer_options['widget_columns'] = GoMMC_Theme_Helper::get_option('widget_columns');
                $footer_options['widget_columns_2'] = GoMMC_Theme_Helper::get_option('widget_columns_2');
                $footer_options['widget_columns_3'] = GoMMC_Theme_Helper::get_option('widget_columns_3');
                $footer_align = GoMMC_Theme_Helper::get_option('footer_align');
        ...
        private function main_footer_get_page()
        {
            ...
            $footer_page_select = GoMMC_Theme_Helper::get_mb_option('footer_page_select', 'mb_footer_switch', 'on');
        ...
        {
            // Get options
            $copyright_spacing = GoMMC_Theme_Helper::get_mb_option('copyright_spacing', 'mb_copyright_switch', 'on');
        ...
        private function copyright_style()
        {
            ...
            if ($this->content_type == 'widgets') {
                $bg_color = GoMMC_Theme_Helper::get_mb_option('copyright_bg_color', 'mb_copyright_switch', 'on');
        ...
        {
            $editor = GoMMC_Theme_Helper::get_option('copyright_editor');

            if ($this->mb_copyright_switch == 'on') {
                $editor = rwmb_meta('mb_copyright_editor');
            }
            ?>
            <div class='copyright' <?php echo GoMMC_Theme_Helper::render_html($this->copyright_style()); ?>>
                <?php if (!$this->footer_full_width) echo "<div class='tpc-container'>"; ?>
                <div class='row' <?php echo GoMMC_Theme_Helper::render_html($this->copyright_spacing()); ?>>
