...
if (!class_exists('GoMMC_Woocoommerce') ) {
    /**
     * GoMMC Woocommerce
     *
     *
     * @package gommc\woocoomerce
     * @author MMC 
     * @since 1.0.0
     */
    class GoMMC_Woocoommerce
    {
        private $row_class;
        ...
        public function setup()
        {
            ...
            add_theme_support(
                'woocommerce',
                apply_filters(
                    'gommc_woocommerce_args',
                    [
                        'single_image_width' => 1140,
                        'thumbnail_image_width' => 480,
                        'gallery_thumbnail_image_width' => 270,
                        'product_grid' => [
                            'default_columns' => (int) GoMMC::get_option('shop_column'),
                            'default_rows' => 4,
                            'min_columns' => 1,
                            'max_columns' => 6,
                            'min_rows' => 1,
        ...
        function review_comments_meta_info($comment)
        {
            ...
            if ('0' === $comment->comment_approved) { ?>
                <em class="woocommerce-review__awaiting-approval">
                    <?php esc_html_e('Your review is awaiting approval', 'gommc'); ?>
                </em>
            ...
            <?php } else { ?>
                ...
                if ('yes' === get_option( 'woocommerce_review_rating_verification_label' ) && $verified) {
                    echo '<em class="woocommerce-review__verified verified">(' . esc_attr__('verified owner', 'gommc') . ')</em> ';
                }
                ?>
                <div class="meta-data">
        ...
        function tpc_filter_comments($comment_form)
        {
            ...
            $comment_form = [
                'title_reply' => have_comments() ? esc_html__('Add a review', 'gommc') : sprintf( esc_html__('Be the first to review &ldquo;%s&rdquo;', 'gommc'), get_the_title() ),
                'title_reply_to' => esc_html__( 'Leave a Reply to %s', 'gommc' ),
                'title_reply_before' => '<h3 id="reply-title" class="comment-reply-title">',
                'title_reply_after' => '</h3>',
                'comment_notes_after' => '',
                'fields' => [
                    'author' => '<p class="comment-form-author">' . '<label for="author"></label> ' .
                    '<input id="author" name="author" placeholder="'.esc_attr__( 'Your Name', 'gommc' ).'" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" required /></p>',
                    'email' => '<p class="comment-form-email"><label for="email"></label> ' .
                    '<input id="email" name="email" placeholder="'. esc_attr__( 'Your Email', 'gommc' ).'" type="email" value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="30" required /></p>',
                ],
                'label_submit' => esc_html__('Submit', 'gommc'),
                'logged_in_as' => '',
                'comment_field' => '',
            ];
            ...
            if ($account_page_url = wc_get_page_permalink('myaccount')) {
                ...
                $comment_form['must_log_in'] = '<p class="must-log-in">' . sprintf( wp_kses( __( 'You must be <a href="%s">logged in</a> to post a review.', 'gommc' ), $allowed_html), esc_url( $account_page_url ) ) . '</p>';
            }

            if (get_option('woocommerce_enable_review_rating') === 'yes') {
                $comment_form['comment_field'] = '<div class="comment-form-rating"><label for="rating">' . esc_html__('Your Rating For This Product', 'gommc') . '</label><select name="rating" id="rating" required>
                <option value="">' . esc_html__('Rate&hellip;', 'gommc') . '</option>
                <option value="5">' . esc_html__('Perfect', 'gommc') . '</option>
                <option value="4">' . esc_html__('Good', 'gommc') . '</option>
                <option value="3">' . esc_html__('Average', 'gommc') . '</option>
                <option value="2">' . esc_html__('Not that bad', 'gommc') . '</option>
                <option value="1">' . esc_html__('Very poor', 'gommc') . '</option>
                </select></div>';
            }

            $comment_form['comment_field'] .= '<p class="comment-form-comment"><label for="comment"></label><textarea id="comment" name="comment" cols="45" rows="8" placeholder="'.esc_attr__('Your review', 'gommc').'" required></textarea></p>';
        ...
        */
        ...
        {
            return (int) GoMMC::get_option('shop_products_per_page');
        }
        ...
        {
            $args = [
                'posts_per_page' => (int) GoMMC::get_option('shop_r_products_per_page'),
                'columns' => (int) GoMMC::get_option('shop_related_columns'),
            ];
        ...
        {
            $columns = (int) GoMMC::get_option('shop_column'); // 3 products per row
        ...
        {
            $columns = (int) GoMMC::get_option('shop_column');
        ...
        public function get_sidebar_data()
        {
            ...
            return GoMMC::get_sidebar_data('shop_' . $shop_template);
        }
        ...
        public function woo_rename_tabs( $tabs ) {
	        global $product;
		    $tabs['reviews']['title'] = esc_html__( 'Reviews', 'gommc' ).' <span class="count">('. $product->get_review_count().')</span>';
		    return $tabs;
	    }
    }
}
...
new GoMMC_Woocoommerce();

// GoMMC Woocoommerce Helpers

if (!function_exists('gommc_woocommerce_breadcrumb')) {
	/**
	 ...
	 * @return string
	 */
    function gommc_woocommerce_breadcrumb($args = [])
    {
        $args = wp_parse_args($args, apply_filters('woocommerce_breadcrumb_defaults', [
            ...
            'before' => '',
            'after' => '',
            'home' => esc_html_x('Home', 'breadcrumb', 'gommc'),
        ]));
        ...
        if (!empty($breadcrumb)) {
            ...
            $out .= GoMMC::render_html($wrap_before);

            foreach ($breadcrumb as $key => $crumb) {

                $out .= GoMMC::render_html($before);
                ...
                $out .= GoMMC::render_html($after);

                if (sizeof($breadcrumb) !== $key + 1) {
                    $out .= GoMMC::render_html($delimiter);
                }
            }
            $out .= GoMMC::render_html($wrap_after);
        }
