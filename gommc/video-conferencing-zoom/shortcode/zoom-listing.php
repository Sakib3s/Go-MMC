...
use TPCAddons\Includes\{
    TPC_Elementor_Helper
};
global $zoom;
...
$image_dims = TPC_Elementor_Helper::get_image_dimensions( '740x540', false );
$tpc_featured_image_url = aq_resize($attachment_url, $image_dims['width'], $image_dims['height'], true, true, true);
if (!$tpc_featured_image_url) {
    $tpc_featured_image_url = $attachment_url;
}
        ...
        <div class="vczapi-list-zoom-meetings--item__details">
            ...
            <div class="vczapi-list-zoom-meetings--item__details__meta">
                <div class="hosted-by meta">
                    <i class="flaticon-user-1"></i> <strong><?php esc_html_e( 'Hosted By', 'gommc' ); ?></strong> <?php echo esc_attr( '&nbsp;:&nbsp;' ); ?> <span> <?php echo apply_filters( 'vczapi_host_name', $zoom['host_name'] ); ?></span>
                </div>
                <?php
                if ( vczapi_pro_version_active() && vczapi_pro_check_type($zoom['api']->type ) ) {
                    ...
                    if ( ! empty( $occurence ) ) {
                        ...
                        <div class="start-date meta">
                            <i class="flaticon-next"></i> <strong><?php esc_html_e( 'Next Occurrence', 'gommc' ); ?><?php echo esc_attr( '&nbsp;:&nbsp;' ); ?></strong>
                        ...
                        ?>
                        <div class="start-date meta">
                            <i class="flaticon-wall-clock"></i> <strong><?php esc_html_e( 'Start Time', 'gommc' ); ?>:</strong>
                    ...
                    ?>
                    <div class="start-date meta">
                        <i class="flaticon-price-tag"></i> <strong><?php esc_html_e( 'Type', 'gommc' ); ?><?php echo esc_attr( '&nbsp;:&nbsp;' ); ?></strong>
                        <span><?php esc_html_e( 'Recurring ', 'gommc' ); ?></span>
                    </div>
                    <?php
                } else {
                    ?>
                    <div class="start-date meta">
                        <i class="flaticon-wall-clock"></i> <strong><?php esc_html_e( 'Start', 'gommc' ); ?><?php echo esc_attr( '&nbsp;:&nbsp;' ); ?></strong>
                        <span><?php echo vczapi_dateConverter( $zoom['api']->start_time, $zoom['api']->timezone, 'F j, Y @ g:i a' ); ?></span>
                    </div>
                <?php } ?>
                <?php if (1>1) { ?>
                    <div class="timezone meta">
                         <i class="flaticon-worldwide"></i> <strong><?php esc_html_e( 'Timezone', 'gommc' ); ?><?php echo esc_attr( '&nbsp;:&nbsp;' ); ?></strong> <span><?php echo esc_html($zoom['api']->timezone); ?></span>
                    </div>
                <?php } ?>
            </div>
