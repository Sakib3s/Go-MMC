...
$sb = GoMMC::get_sidebar_data('single');
$column = $sb['column'] ?? '';
$row_class = $sb['row_class'] ?? '';
$container_class = $sb['container_class'] ?? '';
$layout = $sb['layout'] ?? '';

$single_type = GoMMC::get_mb_option('single_type_layout', 'mb_post_layout_conditional', 'custom') ?: 2;

$row_class .= ' single_type-' . $single_type;

if ('3' === $single_type) {
    echo '<div class="post_featured_bg" style="background-color: ', GoMMC::get_option('post_single_layout_3_bg_image')['background-color'], '">';
...
echo '<div class="row', apply_filters('gommc/row/class', esc_attr( $row_class )), '">';
    echo '<div id="main-content" class="tpc_col-', apply_filters('gommc/column/class', esc_attr( $column )), '">';
        ...
        //* ↓ Related Posts
        $show_post_related = GoMMC::get_option('single_related_posts');
        ...
        if (
            $show_post_related
            && class_exists('GoMMC_Core')
            && class_exists('\Elementor\Plugin')
        ) {
            global $tpc_related_posts;
            $tpc_related_posts = true;

            $mb_blog_carousel_r = GoMMC::get_mb_option('blog_carousel_r', 'mb_blog_show_r', 'custom');
            $mb_blog_title_r = GoMMC::get_mb_option('blog_title_r', 'mb_blog_show_r', 'custom');

            $mb_blog_cat_r = [];

            $cats = GoMMC::get_option('blog_cat_r');
            if (!empty($cats)) {
            ...
            $mb_blog_column_r = GoMMC::get_mb_option('blog_column_r', 'mb_blog_show_r', 'custom');
            $mb_blog_number_r = GoMMC::get_mb_option('blog_number_r', 'mb_blog_show_r', 'custom');
            ...
            <section class="single related_posts"><?php
                ...
                <div class="gommc_module_title">
	                <h4><?php
                        echo !empty($mb_blog_title_r) ? esc_html($mb_blog_title_r) : esc_html__('Related Posts', 'gommc'); ?>
                    </h4>
                </div><?php
                ...
                $related_posts_atts = [
                    ...
                    'img_aspect_ratio' => '',
                    'items_load' => 4,
                    'load_more_text' => esc_html__('Load More', 'gommc'),
